...
class DBSeeder {
    ...
    seed() {
        ...
        const citiesIds = [5, 9, 44, 5, 36, 17, 16, 9, 36, 14, 14, 6, 9, 24, 44, 36, 25, 19, 5, 14, 5, 23, 38, 17];
        ...
        for (let i = 0; i < customerNames.length; i++) {
            const nameGenderHost = customerNames[i].split(',');
            const cityState = citiesStates[i].split(',');
            const state = { 'id': citiesIds[i], 'abbreviation': cityState[1], 'name': cityState[2] };
            ...
            let firstOrder = Math.floor(Math.random() * orders.length);
            let lastOrder = Math.floor(Math.random() * orders.length);
            if (firstOrder > lastOrder) {
                let tempOrder = firstOrder;
            ...
            for (let j = firstOrder; j <= lastOrder && j < orders.length; j++) {
        ...
        for (let i = 0; i < states.length; i++) {
            const state = new State ({ 'id': i + 1, 'name': states[i].name, 'abbreviation': states[i].abbreviation });
            state.save();
        }
    }
}

export default new DBSeeder();
